...
export type StatutCivil = 'celibataire' | 'marie';
export type Religion = 'aucune' | 'reforme' | 'catholique';
export type ClasseAge = 'aucune' | '25-34' | '35-44' | '45-54' | '55-65' | '65-70-actif' | '65-retraite';
...
export interface UserProfile {
  ...
  typeRevenu: 'brut' | 'net';
  classeAge: ClasseAge;
  classeAgeConjoint: ClasseAge;
  religion: Religion;
  pilier3a: string;
  pilier3aConjoint: string;
  fraisGarde: string;
}
...
export interface DeductionPalier {
  nom_deduction: string;
  categorie: string;
  revenu_seuil: number;
...
const CANTON_RULES: Record<number, CantonStrategy> = {
  3: { familySplittingForSingle: true },
  4: { familySplittingForSingle: true },
  5: { familySplittingForSingle: true },
  6: { familySplittingForSingle: true },
  7: { familySplittingForSingle: true },
  8: { familySplittingForSingle: true },
  10: { assuranceStrategy: 'LIRE_MAXIMUM_COMME_GLOBAL', deductionMarieStrategy: 'MONTANT_GLOBAL_SANS_DOUBLER', useProportionalTax: true, familySplittingForSingle: true },
  14: { familySplittingForSingle: true },
  21: { familySplittingForSingle: true },
  23: { familySplittingForSingle: true },
...
export function calculateCotisationsSociales(user: UserProfile): number {
  if (user.typeRevenu === 'net') return 0;
  ...
  const calcLPP = (brutIndividuel: number, ageClass: ClasseAge): number => {
    if (ageClass === '65-retraite' || ageClass === 'aucune') return 0;
    ...
    const tauxLpp: Record<Exclude<ClasseAge, 'aucune' | '65-retraite'>, number> = { 
      '25-34': 0.035, '35-44': 0.05, '45-54': 0.075, '55-65': 0.090, '65-70-actif': 0.090 
    };
  ...
  if (user.statutCivil === 'marie') {
    const r1 = user.revenuBrut - user.revenuConjoint; 
    const r2 = user.revenuConjoint;
    cotisAvsAcLaa = calcCotis1erPilier(r1, user.classeAge === '65-retraite') + calcCotis1erPilier(r2, user.classeAgeConjoint === '65-retraite');
    cotisLpp = calcLPP(r1, user.classeAge) + calcLPP(r2, user.classeAgeConjoint);
  } else {
    const brut = user.revenuBrut;
    cotisAvsAcLaa = calcCotis1erPilier(brut, user.classeAge === '65-retraite');
    cotisLpp = calcLPP(brut, user.classeAge);
  }

  return Math.round(cotisAvsAcLaa + cotisLpp);
}

function calculateNetIncome(user: UserProfile, brut: number): number {
  return Math.max(0, Math.round(brut - user.cotisationsSociales));
}

function getNetConjoint(user: UserProfile, revenuBrut: number): number {
  if (user.statutCivil !== 'marie' || user.revenuConjoint <= 0) return 0;
  const grossConjoint = user.revenuConjoint || 0;
  if (user.typeRevenu === 'net') return grossConjoint;
  if (revenuBrut === 0) return 0;
  return Math.max(0, grossConjoint - (user.cotisationsSociales * (grossConjoint / revenuBrut)));
}

type DeductionResult = { total: number; details: DeductionDetail[] };

function calculerAvecFallback(user: UserProfile, deductionsData: DeductionRow[], statut: StatutCivil, nbEnfants: number, cantonId: number, isMarie2Revenus: boolean, nbMineurs: number, _nbMajeurs: number, revenuNet: number): DeductionResult {
  ...
  if (cantonId === 26) {
    ...
    if (nbEnfants > 0) totalAss += 1020 * nbEnfants;
    ...
    if (max3a > 0) {
      let total3a = Math.min(parseFloat(user.pilier3a) || 0, max3a);
      if (statut === 'marie') total3a += Math.min(parseFloat(user.pilier3aConjoint) || 0, max3a);
      if (total3a > 0) {
  ...
  if (cantonId === 3) {
    ...
    // ==========================================
    if (cantonId === 6) {
      // CORRECTION : 2689 par personne active. Si 1 seul travail, on déduit 2689.
      const fraisProf = isMarie2Revenus ? 5378 : 2689;
      totalDeductions += fraisProf;
      details.push({ nom: 'radar.frais_prof', montant: fraisProf });

      // CORRECTION : La loi dit 3300 pour marié, 1700 pour célibataire (pas 6800 !)
      const assBase = statut === 'marie' ? 3300 : 1700;
  ...
  if (cantonId === 7) {
    ...
    // ==========================================
    if (cantonId === 5) {
      // Les frais pro sont de 6900 par personne active. 
      // Si le conjoint ne travaille pas (isMarie2Revenus = false), on ne déduit que 6900.
      const fraisProf = isMarie2Revenus ? 13800 : 6900;
      totalDeductions += fraisProf;
      details.push({ nom: 'radar.frais_prof', montant: fraisProf });
...
function calculerGenerique(user: UserProfile, deductionsData: DeductionRow[], paliersData: DeductionPalier[], statut: StatutCivil, nbEnfants: number, nbMineurs: number, nbMajeurs: number, revenuNet: number, revenuBrut: number, cantonId: number): DeductionResult {
  let totalDeductions = 0;
  let details: DeductionDetail[] = [];
  let maxDeductionEnfantMineur = 0;
  let maxDeductionEnfantMajeur = 0;
  const rules = CANTON_RULES[cantonId] || {};
  const netConjoint = getNetConjoint(user, revenuBrut);
  ...
  for (const ded of validDeds) {
    ...
    switch (cat) {
      ...
      case 'frais_garde': {
        if (nbEnfants > 0 && parseFloat(user.fraisGarde) > 0) {
          let maxCap = ded.maximum > 0 ? ded.maximum : 0;
          if (isMarie2Revenus) maxCap *= 2;
          totalGarde = Math.max(totalGarde, maxCap > 0 ? Math.min(parseFloat(user.fraisGarde), maxCap * nbEnfants) : parseFloat(user.fraisGarde));
        }
        break;
      }
      ...
      case 'deduction_statut': {
        const nom = ded.nom_deduction.toLowerCase();
        const isUserRetraite = user.classeAge === '65-retraite' || user.classeAgeConjoint === '65-retraite';
...
function calculerTotalDeductions(user: UserProfile, revenuNet: number, revenuBrut: number, deductionsData: DeductionRow[], paliersData: DeductionPalier[], statut: StatutCivil, nbMineurs: number, nbMajeurs: number, cantonId: number): DeductionResult {
  ...
  if (cantonId === 25 || cantonId === 12 || cantonId === 26 || cantonId === 7 || cantonId === 6 || cantonId === 4 ||
    ...
    return calculerAvecFallback(user, deductionsData, statut, nbEnfants, cantonId, isMarie2Revenus, nbMineurs, nbMajeurs, revenuNet);
  }
  return calculerGenerique(user, deductionsData, paliersData, statut, nbEnfants, nbMineurs, nbMajeurs, revenuNet, revenuBrut, cantonId);
}

function calculateTax(revenuImposable: number, bareme: BaremeTranche[]): number {
...
function calculateTaxProportional(revenu: number, bareme: BaremeTranche[]): number {
...
export function calculerImpotPrecis(user: UserProfile, baseRevenuInput: number, baremeCantonal: BaremeTranche[], baremeFederal: BaremeTranche[], commune: Commune, deductionsFed: DeductionRow[], deductionsCant: DeductionRow[], paliersFed: DeductionPalier[], paliersCant: DeductionPalier[], statut: StatutCivil, statutBareme: StatutCivil, nbMineurs: number, nbMajeurs: number): ResultatFiscal {
  const nbEnfants = nbMineurs + nbMajeurs;
  const revenuNet = calculateNetIncome(user, baseRevenuInput);
  const revenuBrut = baseRevenuInput;

  const resultFed = calculerTotalDeductions(user, revenuNet, revenuBrut, deductionsFed, paliersFed, statut, nbMineurs, nbMajeurs, 0);
  const resultCant = calculerTotalDeductions(user, revenuNet, revenuBrut, deductionsCant, paliersCant, statut, nbMineurs, nbMajeurs, commune.canton_id);
  ...
  if (user.religion === 'reforme') impotParoissial = Math.round(basePourCommunal * (commune.coeff_revenu_eglise_reforme / 100));
  if (user.religion === 'catholique') impotParoissial = Math.round(basePourCommunal * (commune.coeff_revenu_eglise_catholique / 100));
...
const RadarFiscal: React.FC<RadarFiscalProps> = ({ initialMode = 'simple', onNextStep, onResultChange, hideWarning }) => {
  ...
  const [enfantsMineurs, setEnfantsMineurs] = useLocalStorageState('radar_enfantsMineurs', 0);
  const [enfantsMajeurs, setEnfantsMajeurs] = useLocalStorageState('radar_enfantsMajeurs', 0);
  ...
  useEffect(() => {
    ...
    const user: UserProfile = {
      revenuBrut: totalBrut(),
      cotisationsSociales: 0,
      statutCivil,
      revenuConjoint: parseFloat(revenuConjoint) || 0,
      typeRevenu,
      classeAge,
      classeAgeConjoint,
      religion,
    ...
    setCotisationsSociales(calculateCotisationsSociales(user).toString());
  }, [revenuInput, typeRevenu, classeAge, classeAgeConjoint, statutCivil, revenuConjoint, setCotisationsSociales]);
  ...
  const handleCompare = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      const statutBaremeCantonal = (statutCivil === 'celibataire' && nombreEnfants > 0) ? 'marie' : statutCivil;

      const user: UserProfile = {
        revenuBrut: input,
        cotisationsSociales: parseFloat(cotisationsSociales) || 0,
        statutCivil,
        revenuConjoint: parseFloat(revenuConjoint) || 0,
        typeRevenu,
        classeAge,
      ...
      const resDep = calculerImpotPrecis(user, input, depData[0].data as BaremeTranche[], depData[1].data as BaremeTranche[], communeDepart, depData[2].data as DeductionRow[], depData[3].data as DeductionRow[], depData[4].data as DeductionPalier[], depData[5].data as DeductionPalier[], statutCivil, statutBaremeCantonal, enfantsMineurs, enfantsMajeurs);
      setResultatDepart(resDep);

      if (mode === 'comparaison' && communeArrivee) {
        const arrData = await Promise.all([
          supabase.from('baremes').select('montant_tranche, taux, montant_base').eq('canton_id', communeArrivee.canton_id).eq('statut', statutBaremeCantonal).eq('autorite_fiscale', 'Canton').order('montant_tranche', { ascending: true }),
        ...
        const resArr = calculerImpotPrecis(user, input, arrData[0].data as BaremeTranche[], depData[1].data as BaremeTranche[], communeArrivee, depData[2].data as DeductionRow[], arrData[1].data as DeductionRow[], depData[4].data as DeductionPalier[], arrData[2].data as DeductionPalier[], statutCivil, statutBaremeCantonal, enfantsMineurs, enfantsMajeurs);
