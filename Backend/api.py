...
GEMINI_KEY = os.getenv("GEMINI_KEY")   # optionnel : si absent, Groq seul
...
# Gemini en principal (via endpoint compatible OpenAI) — repli Groq
gemini_client = OpenAI(
    api_key=GEMINI_KEY,
    base_url="https://generativelanguage.googleapis.com/v1beta/openai/",
) if GEMINI_KEY else None

GEMINI_MODEL = "gemini-3.5-flash"     # le plus récent stable de ta liste
MODEL = "openai/gpt-oss-20b"          # Groq (dernier recours)
...
# Nom complet des langues : un code seul ("en de") ne suffit pas à contraindre le modèle
LANGUE_NOMS = {
    "fr": "français",
...
# Titres de sections fournis directement dans la langue cible
SECTION_HEADERS = {
...
def _get_cached_answer(key: str):
    """RAM d'abord (instantané), puis Supabase (partagé, persistant)."""
    if key in ANSWER_CACHE:
        return ANSWER_CACHE[key]
    try:
        res = (supabase.table("answer_cache")
               .select("answer, fournisseur")
               .eq("cache_key", key).limit(1).execute())
        if res.data:
            answer, fourn = res.data[0]["answer"], res.data[0]["fournisseur"]
            ANSWER_CACHE[key] = (answer, fourn)   # réchauffe la RAM
            print("[CACHE-DB] Réponse trouvée dans Supabase.")
            return (answer, fourn)
...
def _save_cached_answer(key: str, answer: str, fournisseur: str):
    """Écrit dans la RAM (toujours) et dans Supabase (au mieux)."""
...
def health():
    ...
    return {"status": "ok", "fournisseurs": actifs}

@app.post("/api/debug-retrieval")
def debug_retrieval(question_data: Question):
    q_embedding = embed_model.encode(question_data.user_question).tolist()
    response = supabase.rpc('match_tax_docs', {
...
def tax_copilot(question_data: Question):
    try:
        ...
        # 2.5 GARDE-FOU : question probablement non fiscale -> on ne force
        # pas le RAG (évite les réponses hors-sujet et économise les tokens)
        ...
        print(f"[RAG] {len(response.data)} extraits trouvés pour {canton} (similarité top: {top_similarity}).")

        # Garde-fou anti-413 : le contexte ne dépasse jamais 10000 caractères
        MAX_CONTEXT_CHARS = 10000
        if len(context) > MAX_CONTEXT_CHARS:
            context = context[:MAX_CONTEXT_CHARS]
            print(f"[RAG] Contexte tronqué à {MAX_CONTEXT_CHARS} caractères.")
...
Tu dois structurer ta réponse exactement de cette manière, en Markdown :
...
"""
        ...
        last_error = None
        elapsed = 0.0
        TIMEOUT_PAR_FOURNISSEUR = 45.0   # secondes max par fournisseur
        for i, (name, client, model_id, params) in enumerate(providers, start=1):
            try:
                ...
                chat_completion = client.chat.completions.create(
                    ...
                    model=model_id,
                    **params,
                    timeout=TIMEOUT_PAR_FOURNISSEUR,
                )
...
# ============================================================
# ENDPOINT STREAMING — les mots s'affichent dès leur génération.
# Protocole NDJSON (1 objet JSON par ligne) :
#   {"type": "meta", "fournisseur": "..."}   — 1 fois, en premier
#   {"type": "chunk", "text": "..."}         — n fois
#   {"type": "done"}                          — 1 fois, à la fin
# L'ancien /api/tax-copilot reste intact : rollback = 1 ligne côté React.
# ============================================================
...
2. Si le texte source contient des chiffres, des pourcentages ou des montants, cite-les obligatoirement, sans les modifier ni en inventer.
3. Cite le paragraphe ou la loi mentionnée (ex: § 42 Abs. 1 Bst. a StG) si elle est dans le texte, à côté du montant concerné. Ne raccorde jamais une référence à un montant si elle n'est pas adjacente dans l'extrait.
4. RÉPONDS INTÉGRALEMENT en {langue_nom}. Si la langue demandée n'est pas le français, il est interdit d'écrire ne serait-ce qu'une phrase en français.
5. Si les extraits ne contiennent PAS la réponse à la question posée, réponds UNIQUEMENT "Je n'ai pas trouvé cette information dans les lois de ce canton." dans la langue {langue_nom} — sans ajouter de tableau, de déductions ou d'informations non sollicitées.
6. Termine toujours ta réponse complètement, sans jamais t'arrêter en pleine phrase.
